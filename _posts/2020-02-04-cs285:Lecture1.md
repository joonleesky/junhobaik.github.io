...
안녕하세요! 카이스트 인공지능 대학원, 데이터 시각화 연구실의 석사과정에 재학중인 이호준이라고 합니다. 강화학습을 공부하는데 있어 정말 좋은 참고 자료들이 많지만, 이해가 쉽게 되지 않을때면 한글로된 자료들이 부족한것이 아쉬웠던적이 많았던것 같습니다. 이번에 같은 연구실의 이병근 학우와 Sergey Levine교수님의 **CS 285: Deep Reinforcement Learning, Decision Making, and Control**을 공부하며 배운 내용들을 한글로 정리하고자 합니다. 이 강의를 공부하시는 분들에게 조금이라도 도움이 되기를 소망합니다. 노트는 CS 285 강의를 기본으로 하되 흐름에 맞게 내용들을 추가하고 재배치했음을 밝힙니다. 부족한점에 대한 지적과 질문은 자유롭게 댓글로 남겨주세요.
...
**강화학습은 지도학습으로는 쉽게 모델링할 수 없는 우리의 연속된 의사결정들을 모델링하고 평가하기 위한 알고리즘입니다.** 인공지능 모델에게 언제나 정답을 알려주는 직접적인 선생님이 있는것이 아니라, 스스로의 경험을 통해서 어떠한 선택이 올바른 선택이었는지 알아내는 것입니다. 저희의 인공지능 agent는 주어진 environment에서 observation에 따라 action을 통해서 reward을 받고, 그 reward을 합을 최대화 하는 방향으로 학습을 진행하는 것입니다.
...
사실 강화학습이라는 연구 분야는 상당히 오랜시간 동안 각광받지 못해왔던 연구 분야입니다. 그렇다면 도대체 어떠한 요인들로 인해 강화학습이 다시 주목받게 되었을까요? Levine교수님은 3가지 이유를 제시합니다.
