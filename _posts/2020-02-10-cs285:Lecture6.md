...
v_preds: array of value prediction per state
"""
# Temporal difference
...
사실 직접 A2C를 구현하는데 있어서 가장 힘들었던 점은 실제 training 과정이 너무나 unstable하다는 것이었습니다. 강화학습 알고리즘의 benchmark로 종종 사용되는 **OpenAI: Baseline**의 A2C 코드를 살펴보니 actor network의 final layer에서만 weight를 xavier intialization이 아닌 output의 분산이 0.01에 가깝도록 하는 독특한 initialization을 사용하고 있었습니다. Actor network의 output의 variance가 커지게 되면 학습이 채 되기도전에 policy가 converge하게 되고 overfitting에 취약해진다는 사실을 발견하였습니다. 따라서, actor-critic을 구현하는데 있어서 weight initialization을 신경써주실 것을 당부드립니다. 

<center><img src = "/assets/images/cs285/lecture6/init.png" width = "520"></center><div style = "margin-bottom:12px"></div>
<center>From OpenAI: Baseline, A2C implementation</center><div style = "margin-bottom:12px"></div>

## Thoughts

오늘의 강의에서 다룰 내용은 여기까지입니다. 오늘도 정리를 마치기전에 강의에서 잠깐 언급이 되었던 **eligibility trace**에 대한 고민(?)들을 몇자 더 적어봅니다.

### Eligibility trace

앞서 다룬 gae가 n-step advantage들을 average하는 기법이였다면, advantage보다 좀 더 general한 n-step return들을 average하여 return을 추정하는 기법을 **$$\lambda$$-return**이라고 말합니다.

<center><img src = "/assets/images/cs285/lecture6/lambda.png" width = "400"></center><br>

<center><img src = "/assets/images/cs285/lecture6/lambda_fig.png" width = "600"></center>

$$\lambda$$-return은 사실 상당히 직관적입니다. 
